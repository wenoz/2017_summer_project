...
def get_transfer_duration_table():
    ...
    WHERE exit_dateime > 0 and transfer_newservice_provider= 6 and transfer_duration_seconds <= 7200 and transfer_duration_seconds > -300 and 
...
def compute_bus_missed(df):
    ...
    for index in df.index:
        ...
        if df['transfer_route_taken'].loc[index] != rte_so_far :
            ...
            query = "select current_route_id as Route, bs_sname as STOPID, direction AS Direction, apc_date_time as stop_datetime from dssg.pt_avl_temp where current_route_id = " + str(rte_so_far)
            print('rte swtiched to '+ str(rte_so_far))
            temp  = pd.read_sql(query, engine)
            print('reloaded temp table')
...
def main():
    ...
    result.to_csv('~/Desktop/pt_result.csv') 
